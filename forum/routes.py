...
async def get_forum_chat(classroom_uid, token):
    user_enrolled_status = await attendance_controllers.if_user_enrolled(classroom_uid=classroom_uid, user_id=token.user_id)
    if_user_is_creator = await attendance_controllers.check_user_if_creator(classroom_id=classroom_uid, user_id=token.user_id)
